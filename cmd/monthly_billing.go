...
func computeAmountToCharge(fullCentsToCharge float64, monthlyAllowed float64, minRemaining float64) (float64, error) {
	lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("computeAmountToCharge full: %f, monthly allowed: %f, minRemaining: %f\r\n", fullCentsToCharge, monthlyAllowed, minRemaining))
	...
	if monthlyAllowed > 0 && minRemaining < 0 {
		...
		if err != nil {
			lineblocs.Log(logrus.ErrorLevel, fmt.Sprintf("error using ParseFloat on .%f\r\n", percentage))
			lineblocs.Log(logrus.ErrorLevel, err.Error())
			return 0, err
		}

		centsToCharge := math.Ceil(float64(fullCentsToCharge) * percentOfDebit)
		lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("computeAmountToCharge result: %f\r\n", centsToCharge))
...
func MonthlyBilling() error {
	...
	if err != nil {
		lineblocs.Log(logrus.ErrorLevel, "error running query..\r\n")
		lineblocs.Log(logrus.ErrorLevel, err.Error())
		return err
	}
	plans, err := lineblocs.GetServicePlans()
	if err != nil {
		lineblocs.Log(logrus.ErrorLevel, "error getting service plans\r\n")
		lineblocs.Log(logrus.ErrorLevel, err.Error())
		return err
	}
	...
	for results.Next() {
		...
		if err != nil {
			lineblocs.Log(logrus.ErrorLevel, "error getting workspace ID: "+strconv.Itoa(id)+"\r\n")
			continue
		}
		user, err := lineblocs.GetUserFromDB(creatorId)
		if err != nil {
			lineblocs.Log(logrus.ErrorLevel, "error getting user ID: "+strconv.Itoa(id)+"\r\n")
			continue
		}
		...
		if plan == nil {
			lineblocs.Log(logrus.InfoLevel, "No plan found for user..\r\n")
			continue
		}
		...
		if err != sql.ErrNoRows && err != nil {
			lineblocs.Log(logrus.ErrorLevel, "Could not get dids info..\r\n")
			lineblocs.Log(logrus.ErrorLevel, err.Error())
			continue
		}
		...
		for results1.Next() {
			...
			if err != nil {
				lineblocs.Log(logrus.ErrorLevel, "error creating number rental debit..\r\n")
				continue
			}
		...
		if err != nil {
			lineblocs.Log(logrus.ErrorLevel, "error getting base costs..\r\n")
			lineblocs.Log(logrus.ErrorLevel, err.Error())
			continue
		}
		...
		if err != nil {
			lineblocs.Log(logrus.ErrorLevel, "error getting workspace user count.\r\n")
			lineblocs.Log(logrus.ErrorLevel, err.Error())
			continue
		}
		rows.Close()
		...
		lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("Workspace total user count %d", userCount))
		...
		lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("Workspace total membership costs is %f", membershipCosts))
		...
		for results2.Next() {
			...
			lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("scanning in debit source %s\r\n", source))
			switch source {
			case "CALL":
				lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("getting call %d\r\n", moduleId))
				...
				lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("call duration is %d\r\n", duration))
				...
				if err != nil {
					lineblocs.Log(logrus.ErrorLevel, "error getting charge..\r\n")
					lineblocs.Log(logrus.ErrorLevel, err.Error())
					continue
				}
			...
			case "NUMBER_RENTAL":
				lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("getting DID %d\r\n", moduleId))
		...
		lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("Final costs are membership: %f, call tolls: %f, recordings: %f, fax: %f, did rentals: %f, total: %f (cents)\r\n",
			membershipCosts,
			callTolls,
			recordingCosts,
			faxCosts,
		...
		lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("Creating invoice for user %d, on workspace %d, plan type %s\r\n", user.Id, workspace.Id, workspace.Plan))
		...
		if err != nil {
			lineblocs.Log(logrus.ErrorLevel, "error creating invoice..\r\n")
			lineblocs.Log(logrus.ErrorLevel, err.Error())
			continue
		}
		invoiceId, err := res.LastInsertId()
		if err != nil {
			lineblocs.Log(logrus.ErrorLevel, "could not get insert id..\r\n")
			lineblocs.Log(logrus.ErrorLevel, err.Error())
			continue
		}
		lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("Charging user %d, on workspace %d, plan type %s\r\n", user.Id, workspace.Id, workspace.Plan))
		...
		if plan.PayAsYouGo {
			...
			if charge == totalCosts { //user has enough credits
				lineblocs.Log(logrus.InfoLevel, "User has enough credits. Charging balance\r\n")
				...
				if err != nil {
					lineblocs.Log(logrus.ErrorLevel, "error updating debit..\r\n")
					lineblocs.Log(logrus.ErrorLevel, err.Error())
					continue
				}
			} else {
				lineblocs.Log(logrus.InfoLevel, "User does not have enough credits. Charging balance as much as possible\r\n")
				...
				lineblocs.Log(logrus.InfoLevel, "Charging remainder with card..\r\n")
				...
				if err != nil {
					...
					lineblocs.Log(logrus.InfoLevel, fmt.Sprintf("variable retryAttempts is setup incorrectly. Please ensure that it is set to an integer. retryAttempts=%s setting value to 0", billingParams.Data["retry_attempts"]))
					retryAttempts = 0
				}
				...
				case "stripe":
					...
					if err != nil {
						lineblocs.Log(logrus.ErrorLevel, "error charging user..\r\n")
						lineblocs.Log(logrus.ErrorLevel, err.Error())
						continue
					}
				case "braintree":
			...
			lineblocs.Log(logrus.InfoLevel, "Charging recurringly with card..\r\n")
			...
			if err != nil {
				...
				if err != nil {
					lineblocs.Log(logrus.ErrorLevel, "error updating invoice....\r\n")
					lineblocs.Log(logrus.ErrorLevel, err.Error())
					continue
				}
